...
func TestListIssueEvents(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprintln(w, issueEventsJson)
	}))
	defer ts.Close()
	...
	issue := Issue{
		...
		EventsUrl: fmt.Sprintf("%s/issues/1/events", ts.URL),
		Submitter: "tester1",
	}
	issueEvents, err := gh.ListIssueEvents(dummyLogger(t), &issue)

	assert.NoError(t, err)
	assert.Len(t, issueEvents, 3)
	assert.Equal(t, issueEvents[0].ActorId, "tester1")
	assert.Equal(t, issueEvents[0].EventType, IssueCreated)
	assert.Equal(t, issueEvents[1].ActorId, "tester1")
	assert.Equal(t, issueEvents[1].EventType, IssueLabeled)
	assert.Equal(
		t,
		(issueEvents[1].Detail.(map[string]interface{}))["name"].(string),
		"ready",
	)
	assert.Equal(t, issueEvents[2].ActorId, "tester2")
	assert.Equal(t, issueEvents[2].EventType, IssueMerged)
	assert.Equal(t, issueEvents[2].Detail.(string), "fafafa1234678")
}
